...
export const metadata: Metadata = generateMeta({
  ...
  path: '/retrato-corporativo',
});

export default function RetratoCorporativoPage() {
  return (
    <>
      <SchemaOrg
      ...
      <section className="bg-bg pb-20 pt-32 md:pb-28 md:pt-44">
        <div className="mx-auto max-w-7xl px-6 md:px-12">
          <Breadcrumbs
            items={[
              { name: 'Início', path: '/' },
              { name: service.title, path: '/retrato-corporativo' },
            ]}
          />
      ...
      {service.packages && (
        <section className="bg-bg-section py-20 md:py-28">
          <div className="mx-auto max-w-7xl px-6 md:px-12">
            ...
            <h2 className="mt-4 font-heading text-3xl font-bold uppercase text-white md:text-4xl">Pacotes de retrato corporativo</h2>
            <p className="mt-4 max-w-2xl font-body text-sm text-text-muted">
              Os pacotes abaixo são apenas referência de escopo. O investimento é sempre
              calculado a partir de um briefing personalizado — solicite um orçamento.
            </p>
            <div className="mt-12 grid grid-cols-1 gap-6 md:grid-cols-3">
              {service.packages.map((pkg) => (
                <div key={pkg.name} className="rounded-2xl border border-border-dark bg-bg-card p-8">
                  <h3 className="font-heading text-xl font-bold text-white">{pkg.name}</h3>
                  <p className="mt-4 font-body text-sm text-text-muted">{pkg.description}</p>
                </div>
              ))}
            </div>
          </div>
